...
Parameter names are used in the rewrite portion without the `@` prefix. [The `@` is a signal to `glue` to unpack the tree sub-match in a recursive manner - see `Ohm-JS` documentation for details on how to do this manually.]
